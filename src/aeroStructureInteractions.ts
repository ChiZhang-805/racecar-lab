  ...
  return [
    ...
    },
    {
      ...
      evaluate: values => {
        ...
        const mismatch = Math.abs(frontTime - rearTime + delay)
        const balanceStep = bounded(mismatch / 450) * (gp ? 12 : 8), yawRisk = bounded(mismatch / 300 + Math.max(0, speed - (gp ? 250 : 90)) / (gp ? 400 : 180))
        const front = curve(t => bounded(t / frontTime), 0, 650), rear = curve(t => bounded((t - delay) / rearTime), 0, 650)
        return safeResult({ metrics: [metric('mismatch', '状态错配时间', 'State mismatch', mismatch, 'ms', warnTone(mismatch, 80, 180)), metric('balance-step', '气动平衡跳变', 'Aero-balance step', balanceStep, '%'), metric('transition-limit', '400 ms 余量', '400 ms margin', gp ? 400 - Math.max(frontTime, rearTime) : 0, 'ms', gp ? lowTone(400 - Math.max(frontTime, rearTime), 60, 0) : undefined), metric('yaw-risk', '姿态风险', 'Attitude risk', yawRisk * 100, '%', warnTone(yawRisk, 0.55, 0.8))], points: front, secondaryPoints: rear, insight: gp ? l('2026 两翼固定状态必须由位置传感确认并在受控时间内协同完成；命令到达不等于机械实位。', 'The fixed 2026 states require sensed position and coordinated timing; a received command is not mechanical position.') : l('学生车没有赛道主动翼；此实验只演示若两个手动设定不一致会怎样破坏平衡。', 'The Student car has no on-track active aero; this only illustrates inconsistent manual setup.'), visual: { labels: [l('前翼命令', 'Front command'), l('前翼实位', 'Front position'), l('尾翼实位', 'Rear position'), l('平衡', 'Balance')], values: [1, bounded(1 - mismatch / 500), bounded(1 - mismatch / 500), 1 - yawRisk], marker: bounded(Math.max(frontTime, rearTime) / 500), risk: yawRisk } })
      },
    },
    {
  ...
  return [
    ...
    parameters: [parameter('speed', '车速', 'Speed', gp ? 60 : 15, gp ? 350 : 120, 5, gp ? 240 : 65, 'km/h'), parameter('throat', '喉口高度', 'Throat height', gp ? 18 : 25, gp ? 55 : 75, 1, gp ? 32 : 45, 'mm'), parameter('ratio', '入口/喉口面积比', 'Inlet/throat area ratio', 1.05, gp ? 1.9 : 1.75, 0.01, gp ? 1.5 : 1.35, '—'), parameter('inlet', '入口来流质量', 'Inlet-flow quality', 50, 100, 1, gp ? 82 : 85, '%')],
      evaluate: values => {
        ...
        const vin = speed / 3.6, optimum = gp ? 31 : 44, choke = bounded(Math.exp(-(((throat - optimum) / (gp ? 17 : 23)) ** 2)) * (throat < (gp ? 23 : 32) ? throat / (gp ? 23 : 32) : 1)), vt = vin * ratio * (0.72 + 0.28 * inlet)
        const dp = Math.max(0, 0.5 * 1.18 * (vt ** 2 - vin ** 2) * choke), load = dp * (gp ? 1.4 : 0.9) * inlet
        return safeResult({ metrics: [metric('throat-speed', '喉口速度', 'Throat speed', vt, 'm/s'), metric('pressure-drop', '压力降', 'Pressure drop', dp, 'Pa'), metric('floor-load', '底板下压力', 'Floor downforce', load, 'N'), metric('margin', '节流/分离裕度', 'Choke/separation margin', choke * 100, '%', lowTone(choke, 0.65, 0.4))], points: curve(h => { const c = bounded(Math.exp(-(((h - optimum) / (gp ? 17 : 23)) ** 2)) * (h < (gp ? 23 : 32) ? h / (gp ? 23 : 32) : 1)); return Math.max(0, 0.5 * 1.18 * ((vin * ratio) ** 2 - vin ** 2) * c) }, gp ? 18 : 25, gp ? 55 : 75), insight: l('收缩提高局部速度，但过小高度会进入节流、泄漏、分离或触地风险，载荷必须出现拐点。', 'Contraction raises local speed, but too little height causes choking, leakage, separation or strike; load must have a knee.'), visual: { labels: [l('入口', 'Inlet'), l('喉口', 'Throat'), l('扩散器', 'Diffuser'), l('出口', 'Outlet')], values: [inlet, choke, bounded(dp / (gp ? 6000 : 1600)), bounded(choke * inlet)], risk: 1 - choke } })
      },
    },
    {
  ...
  return [
    {
      ...
      parameters: [parameter('mass', '等效质量', 'Equivalent mass', gp ? 800 : 250, gp ? 950 : 350, 5, gp ? 900 : 300, 'kg'), parameter('speed', '碰撞速度', 'Impact speed', gp ? 10 : 4, gp ? 20 : 9, 0.1, gp ? 17 : 7, 'm/s'), parameter('stroke', '有效压溃行程', 'Effective crush stroke', gp ? 0.35 : 0.15, gp ? 0.85 : 0.35, 0.01, gp ? 0.65 : 0.25, 'm'), parameter('force', '平均压溃力', 'Mean crush force', gp ? 80 : 15, gp ? 260 : 45, 1, gp ? 200 : 29.4, 'kN')],
      evaluate: values => {
        const mass = read(values, 'mass', gp ? 900 : 300), speed = read(values, 'speed', gp ? 17 : 7), stroke = read(values, 'stroke', gp ? 0.65 : 0.25), force = read(values, 'force', gp ? 200 : 29.4)
    ...
    },
    {
      ...
      parameters: [parameter('firstEnergy', '第一次冲击能量', 'First-event energy', 0, gp ? 130 : 10, gp ? 2 : 0.2, gp ? 75 : 2, 'kJ'), parameter('remainingLength', '剩余结构长度', 'Remaining structure', 0, 100, 1, 100, '%'), parameter('secondSpeed', '第二次速度', 'Second-event speed', gp ? 8 : 0, gp ? 17 : 8, 0.1, gp ? 14 : 0, 'm/s'), parameter('retention', '连接保持率', 'Attachment retention', 40, 100, 1, gp ? 95 : 100, '%')],
      evaluate: values => {
        const first = read(values, 'firstEnergy', gp ? 75 : 2), length = read(values, 'remainingLength', 100) / 100, speed = read(values, 'secondSpeed', gp ? 14 : 0), retention = read(values, 'retention', gp ? 95 : 100) / 100, mass = gp ? 900 : 300, nominal = gp ? 145 : 9
        ...
        return safeResult({ metrics: [metric('remaining', '剩余吸能容量', 'Remaining capacity', remaining, 'kJ'), metric('residual', '第二事件残余能量', 'Second-event residual energy', residual, 'kJ', warnTone(residual, gp ? 5 : 1, gp ? 20 : 3)), metric('peak-g', '第二事件峰值', 'Second-event peak', peakG, 'g'), metric('cell-risk', '生存舱输入风险', 'Cell-input risk', risk * 100, '%', warnTone(risk, 0.45, 0.7))], points: curve(t => Math.max(0, nominal - first * bounded(t / 1.0)), 0, 1), secondaryPoints: curve(t => t < 0.55 ? secondEnergy : Math.max(0, secondEnergy - remaining * (t - 0.55) / 0.45), 0, 1), insight: gp ? l('该图只解释 2026 两阶段 FIS 的规定测试逻辑，不能认证真实连续事故。', 'This only explains the prescribed 2026 two-stage FIS test logic; it cannot certify a real sequential accident.') : l('学生车 IA 在任何撞击后都必须按程序检查或更换；界面绝不建议重复使用。', 'A Student IA must be inspected or replaced after any impact; this interface never recommends reuse.'), visual: { labels: [l('第一次事件', 'First event'), l('剩余结构', 'Remaining structure'), l('第二次事件', 'Second event'), l('舱体输入', 'Cell input')], values: [bounded(first / nominal), bounded(remaining / nominal), bounded(secondEnergy / nominal), risk], risk } })
      },
    },
  ]
}
  ...
  return [
    ...
    },
    {
      ...
      evaluate: values => {
        ...
        // A support cannot carry tension in this contact-only teaching model.
        // Saturate the lateral-moment bias at lift-off rather than clipping one
        // reaction independently and losing vertical-force equilibrium.
        const bias = bounded(requestedBias, -rearBase, rearBase)
        const left = rearBase - bias
        const right = rearBase + bias
        const imbalance = div(Math.abs(left - right), Math.max(1, left + right))
        const displacement = Math.max(front / fk, left / rk, right / rk)
        return safeResult({ metrics: [metric('front', '前节点反力', 'Front reaction', front, 'kN'), metric('rear-left', '左后反力', 'Left-rear reaction', left, 'kN'), metric('rear-right', '右后反力', 'Right-rear reaction', right, 'kN'), metric('displacement', '最大安装位移', 'Maximum mount displacement', displacement, 'mm')], points: curve(e => bounded(rearBase + total * e / (gp ? 1200 : 900) * Math.sin(direction), 0, 2 * rearBase), gp ? -220 : -180, gp ? 220 : 180), insight: gp ? l('GP 显示一个前、两个后安装点；偏心矩把一侧卸载到零后即达到接触约束，三点反力之和仍等于外载。FIA 前/后安装计算工况必须分别叠加。', 'GP shows one front and two rear mounts. Once eccentric moment unloads one side to zero, the contact constraint saturates while all three reactions still sum to the applied load; FIA front/rear cases remain separate overlays.') : l('学生视图使用前/主防滚架及支撑节点；节点卸载到零后会约束重分配，反力和仍守恒，且绝不显示三个 Halo 安装脚。', 'Student mode uses front/main roll-hoop and brace nodes; lift-off triggers constrained redistribution while reaction sum remains conserved, and it never shows three Halo feet.'), visual: { labels: [l('前节点', 'Front node'), l('左后节点', 'Left rear'), l('右后节点', 'Right rear'), l('舱体', 'Cell')], values: [bounded(front / total * 2), bounded(left / total * 2), bounded(right / total * 2), bounded(1 - displacement / 5)], risk: bounded(imbalance + displacement / 10), direction: bounded(offset / (gp ? 220 : 180), -1, 1) } })
      },
    },
    {
