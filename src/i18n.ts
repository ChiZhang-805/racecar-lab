...
export const copy = {
  zh: {
    ...
    observeLabels: ['先定位', '再观察', '最后验证'], engineeringLabels: ['关键关系', '设计取舍', '整车影响'], faultLabels: ['失效现象', '工程后果'],
  ...
  en: {
    ...
    observeLabels: ['Locate', 'Inspect', 'Verify'], engineeringLabels: ['Key relation', 'Design trade-off', 'Vehicle effect'], faultLabels: ['Failure sign', 'Engineering result'],
...
const enParts: Record<PartId, Partial<PartInfo>> = {
  ...
  halo: {
    short: 'A Formula Student cockpit-protection system built around the front and main roll hoops, braces, head restraint and harness.',
    purpose: 'Maintains survival space around the helmet in a roll-over or impact and keeps the driver within the protected envelope.',
    analogy: 'A load-bearing safety cage around the driver, with the head restraint and harness keeping the person inside its protection.',
    observe: ['Check from the side that the helmet remains inside the envelope formed by the front and main roll hoops.', 'Trace the main-hoop braces and mounts into the chassis.', 'Inspect the seat, head restraint, harness paths and driver-egress space.'],
    engineering: ['Roll-hoop strength, brace geometry and a continuous load path must satisfy the safety requirement together.', 'Seat, head restraint, harness and driving posture jointly control head and torso motion in an incident.', 'The protection envelope must also preserve normal visibility, control space and rapid egress.'],
    faults: ['Damage at weld toes, braces or mounts weakens the roll-over load path.', 'A helmet outside the protected envelope or poor restraint position increases head and neck injury risk.'], connections: ['Chassis & monocoque', 'Seat, head restraint & harness'],
  },
  tires: {
    short: 'Longitudinal and lateral forces exchanged with the road during acceleration, braking and cornering pass through four small contact patches.',
  ...
  brakes: {
    ...
    analogy: 'Energy does not disappear; the brakes convert vehicle kinetic energy into heat in the discs.',
    ...
    engineering: ['Brake balance follows load transfer and aero load.', 'Single-wheel brake torque is approximately T_b ≈ μ F_clamp R_eff, set by friction coefficient, caliper clamp force and effective friction radius.', 'Poor cooling causes fade; low temperature can also reduce friction.'],
...
const enCourses: Record<CourseId, Partial<Course>> = {
  orientation: { title: 'Meet the race car', subtitle: 'Build the whole-car map', description: 'Find the major systems and see how they coordinate around the tires.', task: 'Locate and isolate the front wing, monocoque and tires.', question: 'What transfers longitudinal and lateral force between the car and road during acceleration, braking and cornering?', options: ['Chassis', 'Air', 'Tires', 'Battery'] },
  ...
  braking: { title: 'Brake system', subtitle: 'Turn kinetic energy into heat', description: 'Observe disc temperature, axle load transfer and the stability effect of brake balance.', task: 'Use Braking mode, hide a tire and inspect disc temperature.', question: 'What is the main energy conversion in a brake disc?', options: ['Heat to kinetic energy', 'Kinetic energy to heat', 'Electricity to potential energy', 'Pressure to electricity'] },
