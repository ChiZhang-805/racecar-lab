...
const O: Record<PartId, Override> = {
  ...
  inverter: {
    ...
    formulas: [
      f('驱动变换效率','Drive conversion efficiency','ηinv = Pac / Pdc',String.raw`\eta_{\mathrm{inv}}=\frac{P_{\mathrm{ac}}}{P_{\mathrm{dc}}}`,'Pac 为三相输出有功功率；Pdc 为直流输入功率','Pac three-phase active output; Pdc DC input power','回收方向必须反转功率比并统一符号，损耗始终为正。','In regeneration, invert the power ratio and keep signs consistent while loss remains positive.'),
  ...
  motor: {
    overview: l('2026 顶级混动动力单元由 1.6 L、90°夹角、六缸涡轮增压内燃机和 MGU-K 组成，不再包含 MGU-H。按 FIA 2026 年 4 月公布的修订说明，MGU-K 在关键加速区可达 350 kW、其他区域限制为 250 kW；内燃机、电机、电池与热约束仍需按整圈协调。', 'The 2026 grand prix power unit combines a 1.6 L, 90-degree, six-cylinder turbocharged ICE and MGU-K without MGU-H. Under the FIA refinement published in April 2026, MGU-K may reach 350 kW in key acceleration zones and is limited to 250 kW elsewhere; engine, motor, battery and thermal constraints still require lap-level coordination.'),
    ...
    concepts: three(['内燃机与 MGU-K 扭矩必须在换挡、牵引和制动回收中连续协调。','ICE and MGU-K torque must coordinate through shifts, traction and regeneration.'],['没有 MGU-H 后，涡轮瞬态和电助力分配更加关键；MGU-K 的 350/250 kW 区域边界还会改变每段直道的能量决策。','Without MGU-H, turbo transient response and electrical boost allocation matter more; the 350/250 kW MGU-K zone boundary also changes the energy decision on each straight.'],['最大瞬时功率不是整圈最优，SOC和热状态具有跨弯道记忆。','Maximum instant power is not lap-optimal; SOC and thermal state carry memory across corners.']),
    formulas: [
