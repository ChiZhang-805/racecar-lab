...
- `public/audio/manifest.json` 固定每个音频二进制的 SHA-256 与技术参数；当前仅 1 首有已核验的一手条目页，其余 7 首在补齐原始下载页与许可快照前不得视为已完成再分发权利核验
...
- `src/music.ts`：背景音乐歌单、文件路径、原始文件名和权利归档提醒；未核验的来源不写入猜测链接
...
需要 Node.js 20.19+，或 22.12+。仓库提供 `.nvmrc`，使用 nvm 时可直接执行 `nvm use`。
...
- `npm run test:browser` 会先构建生产包，再由 Vite Preview 提供与部署产物一致的静态页面，并在真实 Chromium 中检查四种目标分辨率、语言隔离、车型与课程状态、36 套 3D 模型、爆炸交互、公式、实时计算和主要界面的 WCAG 自动化规则。设置 `PLAYWRIGHT_BASE_URL` 可改为直接复测已部署站点。
...
- Web 服务：Nginx
- 带内容哈希的 `/assets/`：gzip 与一年不可变缓存
- 文件名稳定的 `/audio/`、`/images/`：每次复用前向服务器重新验证，避免覆盖同名媒体后仍命中旧版本
- HTML：不缓存，未知路径回退到 `index.html`
- 安全：CSP、禁止 iframe、MIME 嗅探保护、权限策略和同源资源策略响应头

仓库内的 `deploy/deploy-racecar-lab.sh` 与 `deploy/racecar-lab.nginx.conf` 是部署的唯一受版本控制入口。脚本使用独立暂存目录、归档路径校验、Nginx 配置预检、原子软链接切换和失败自动回滚；默认保留当前/上一版本及最近共 5 个发布，不会清理任何无关应用或目录。
