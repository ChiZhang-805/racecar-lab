...
- 当前线上应用提交：`9794e7d`（`fix: equalize driver card heights`）
- 当前线上 release：`20260716T104108Z_9794e7d`
- 上一回滚 release：`20260716T095634Z_4241607`
- 最终验证：Vitest `73/73`、Playwright `13/13`、TypeScript 与 Vite 生产构建通过
- 公网冒烟：中英文四张车队卡均为 `669 px`，每张卡内两位车手均为 `301.5 px`，底部间距均为 `5 px`；8 张图片全部加载，横向溢出 `0`，浏览器错误 `0`
- 本轮完整备份：`C:\Users\zju20\Documents\Codex\2026-07-16\yue\outputs\racecar-lab-driver-height-fix-2026-07-16`

本文件之后如果仅有交接/备份类文档提交，线上应用仍对应 `9794e7d`；不要把纯文档提交误报为已重新部署的页面版本。
...
RaceCar Lab 当前是一个已上线、可回滚、经过 73 个单元测试和 13 个真实浏览器测试验证的互动赛车工程学习站；车队技术车库现已具备大字号、三栏全宽、复杂车型 SVG、四队八位车手，以及不受中英文文案长度影响的桌面等高卡片和完整移动端滚动体验，后续应继续提高公开证据质量、工程交互深度与长期发布合规性。
