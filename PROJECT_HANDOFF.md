...
- 当前线上应用提交：`04b47bf`（`fix: reveal full garage study car`）
- 当前线上 release：`20260716T130635Z_04b47bf`
- 上一回滚 release：`20260716T122642Z_884f6d3`
- 最终验证：Vitest `74/74`、Playwright `13/13`、TypeScript 与 Vite 生产构建通过
- 公网回归：Playwright 车型研究矩阵通过；1440×900 和 390×844 覆盖中英文与两类车型。Mercedes 头图两只轮胎均完整落在容器内并保留底部间距，页面无横向溢出、浏览器错误 `0`
- 本轮完整备份：`C:\Users\zju20\Documents\Codex\2026-07-16\yue\outputs\racecar-lab-garage-hero-fix-2026-07-16`

本文件之后如果仅有交接/备份类文档提交，线上应用仍对应 `04b47bf`；不要把纯文档提交误报为已重新部署的页面版本。
...
- 车型研究“当前研究车”头图提升至 160 px，赛车绘图区为 138 px，并在桌面上提 8 px；前后轮、前翼和尾翼完整可见。右侧标签、车型标题和说明分别至少为 13 px、32 px、17 px；工程问题、事实卡、来源区和间距同步压缩以保持同屏信息密度。
